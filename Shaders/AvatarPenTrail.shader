...
// 2022/07/01 Modified by AoiKamishiro @aoi3192

Shader "Hidden/U-Stella/AvatarPenTrail"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(triangle appdata IN[3], inout TriangleStream<v2f> triStream)
            {
                ...
                float range = max(distance(wpos0, wpos1), max(distance(wpos1, wpos2), distance(wpos2, wpos0)));
                if (range < _ClipRange)
                {
                    v2f o;
